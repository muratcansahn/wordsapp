...
const getAdUnitId = (testId: string, prodId: any, adIndex: number = 0): string => {
  ...
  // Validate prodId exists
  if (!prodId) {
    console.warn('[AdMob] No production adUnitId provided, falling back to test ID');
    return testId;
  }
  ...
  return prodId || testId; // Fallback to testId if prodId is falsy
};
...
export const useAdmobRewarded = (adIndex: number = 0) => {
  ...
  useEffect(() => {
    // Validate adUnitId before creating the ad
    if (!adUnitId || typeof adUnitId !== 'string') {
      console.error('[AdMob] Invalid adUnitId:', adUnitId);
      return;
    }
    ...
    try {
      ...
      console.error('[AdMob] RewardedAd creation failed:', error);
    }
  }, []);
